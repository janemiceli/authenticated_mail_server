...
namespace LumiSoft.Net.SMTP.Server
{
	...
	{				
        ...
        private bool m_CERT_mode = false;  //send default to encyption mode - - miceli
        private bool m_CERT_valid = false; // miceli
        private static object msgLocker = new object();
		...
		private void BeginRecieveCmd()
		{
			...
			this.Socket.BeginReadLine(strm,102400,strm,new SocketCallBack(this.EndRecieveCmd));
		}
        ...
        # region function CERT
        private void CERT(String ArgsText)  // miceli : exchange certificate
        // DO MORE CODING HERE
			...
			*/
			...
			if(param.Length > 0 && param.Length < 3){				
				if(Core.IsNumber(param[0])){
                    int countToRead = Convert.ToInt32(param[0]); //must see what this gets set too miceli
                    ...
                    else
                    {
                        ...
                        //SaveStream(m_pMsgStream, @"C:\receivedBDAT.txt");
                    }
