...
namespace LumiSoft.Net.POP3.Client
{
	...
	public class POP3_Client : IDisposable
	{
		...
		{
			if(!m_Connected){
				pop3Clnt = new Socket(AddressFamily.InterNetwork,SocketType.Stream,ProtocolType.Tcp);
				IPEndPoint ipdest = new IPEndPoint(System.Net.Dns.Resolve(host).AddressList[0],port);
				pop3Clnt.Connect(ipdest);

				// Set connected flag
				m_Connected = true;

				string reply = Core.ReadLine(pop3Clnt);
		...
		public void Authenticate(string userName,string password,bool tryApop)
		{
			...
			if(tryApop && m_ApopHashKey.Length > 0){
				...
				Core.SendLine(pop3Clnt,"APOP " + userName + " " + hexHash);
				...
				Core.SendLine(pop3Clnt,"USER " + userName);

				string reply = Core.ReadLine(pop3Clnt);
				if(reply.StartsWith("+OK")){
					Core.SendLine(pop3Clnt,"PASS " + password);

					reply = Core.ReadLine(pop3Clnt);
					if(reply.StartsWith("+OK")){
						m_Authenticated = true;
					}
					else{
		...
		public POP3_MessagesInfo GetMessagesInfo()
		{
			...
			*/
			...
			string line = Core.ReadLine(pop3Clnt);
			if(line.StartsWith("+OK")){
				// Read lines while get only '.' on line itshelf.
				while(true){
					line = Core.ReadLine(pop3Clnt);
		...
		public void GetMessage(int nr,Socket socket)
		{
			...
			Core.SendLine(pop3Clnt,"RETR " + nr.ToString());

			// Read first line of reply, check if it's ok
			string line = Core.ReadLine(pop3Clnt);
			if(line.StartsWith("+OK")){
				NetworkStream readStream  = new NetworkStream(pop3Clnt);
		...
		public byte[] GetMessage(int nr)
		{
			...
			string line = Core.ReadLine(pop3Clnt);
			if(line.StartsWith("+OK")){
				return Core.DoPeriodHandling(Core.ReadData(pop3Clnt,"\r\n.\r\n",".\r\n"),false).ToArray();
			}
			else{
				throw new Exception("Server returned:" + line);
			}
		...
		public void DeleteMessage(int messageNr)
		{
			...
			Core.SendLine(pop3Clnt,"DELE " + messageNr.ToString());
		...
		public byte[] GetTopOfMessage(int nr,int nLines)
		{
			...
			Core.SendLine(pop3Clnt,"TOP " + nr.ToString() + " " + nLines.ToString());
