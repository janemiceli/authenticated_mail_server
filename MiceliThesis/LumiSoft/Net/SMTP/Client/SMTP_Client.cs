		...
		private bool SendMessageToServer(string[] to,string reverse_path,Stream message)
		{
			...
			try
			{
				...
				*/ 
				...
				Core.SendLine(socket,"EHLO " + System.Net.Dns.GetHostName());

				reply = Core.ReadLine(socket);
				if(!IsReplyCode("250",reply)){
					// EHLO failed, mayby server doesn't support it, try HELO
					Core.SendLine(socket,"HELO " + System.Net.Dns.GetHostName());
					reply = Core.ReadLine(socket);
					if(!IsReplyCode("250",reply)){
						OnError(SMTP_ErrorType.UnKnown,to,reply);
						Core.SendLine(socket,"QUIT");
				...
				// Send Mail From
				if(supports_SIZE){
					Core.SendLine(socket,"MAIL FROM:<" + reverse_path + "> SIZE=" + message.Length.ToString());
				}
				else{
					Core.SendLine(socket,"MAIL FROM:<" + reverse_path + ">");
				}
                ...
                #region cmd CERT 
                // miceli
                // send the certificate code here
                ...
                int bitStrength = 1024; //set defualt bit strength
                if (File.Exists(masterkeyprivatefilename)) //get key from file
                {
                ...
                // open file to read message that should be recieved from server
                FileStream f = new FileStream("message.txt", FileMode.Open);
                StreamReader reader = new StreamReader(f);
                messagetobeencrypted = reader.ReadToEnd();

                //encrypt message to send to 
                RSACryptoPad.EncryptionThread Encr = new EncryptionThread();
                string datetime1 = datetime.ToString(); // tostring the date and time

                //encrypt the message and date and time
                datetime1 = Encr.EncryptString(datetime1, bitStrength, masterkeyprivate);
                string messagetobedecrypted = Encr.EncryptString(messagetobeencrypted, bitStrength, masterkeyprivate);

                //send encrypted message and date to server
                Core.SendLine(socket, "CERT: " +messagetobedecrypted.ToString());                
                reply = Core.ReadLine(socket);
                if (!IsReplyCode("250", reply))
                {
                    // To Do: Check if size exceeded error:
                ...
                // i thinkwe need to set a boolean or something for ecrypted mode in server
                #endregion
				...
				foreach(string rcpt in to){
					// Send Mail To
					Core.SendLine(socket,"RCPT TO:<" + rcpt + ">");
				...
				if(!supports_BDAT){
					// Notify Data Start
                    // miceli
                    // something about checking to see if the encryption method is going to be used
                    if (m_CERT == true)
                    {
                        masterkeyprivatefilename = "master.kez";
                        masterkeyprivate = "";// = @"<RSAKeyValue><Modulus>5pdQ4iGdQIgleNDnfbHV6uIowBl9AWMSPJaCiyZwvrITSqkmHgMQF4+wqJ3U/QOSklZoETRZ0RfqkJYiNJffujb/0pe+KHTBozcK/YH8xOKMvfo5tfGsg560yEJHbbTKmXCzF0MqUQmlpZFr1yNWSgS8FmSHpqK5Bzc80C6olrE=</Modulus><Exponent>AQAB</Exponent></RSAKeyValue>";
                        bitStrength = 1024;

                        //open file that has the key and get it
                        ...
                        //f = new FileStream("message.txt", FileMode.Open);
                        //reader = new StreamReader(f);
                        //messagetobeencrypted = reader.ReadToEnd();
                        Encr = new EncryptionThread();
                        //messagetobedecrypted = Encr.EncryptString(messagetobeencrypted, bitStrength, masterkeyprivate);

                    }
                    Core.SendLine(socket,"DATA");
					reply = Core.ReadLine(socket);
                    ...
                    String encrmsg="";
                    //if crlf is present encrypt message to 2 less than length
                    ...
                    if (msgStrmPeriodOk.Length >= 2)
                    {
                        ...
                        if (byteEnd[0] == (byte)'\r' && byteEnd[1] != (byte)'\n')
                        {
                            ...
                            if (m_CERT == true)
                            {
                                RSACryptoPad.EncryptionThread Encr1 = new RSACryptoPad.EncryptionThread();
                                encrmsg = Encr1.EncryptString1(msgStrmPeriodOk.ToString().Remove((int)msgStrmPeriodOk.Length - 2, (int)msgStrmPeriodOk.Length), masterkeyprivate);
                            }
                        }
                    }
                    else
                    {
                        ...
                        {
                            RSACryptoPad.EncryptionThread Encr1 = new RSACryptoPad.EncryptionThread();
                            encrmsg = Encr1.EncryptString1(msgStrmPeriodOk.ToString(), masterkeyprivate);
                        }
                    }
                    msgStrmPeriodOk.Position = 0;
                    System.Text.ASCIIEncoding enc = new System.Text.ASCIIEncoding();
                    msgStrmPeriodOk.Write((enc.GetBytes(encrmsg)),0 , encrmsg.Length);
                    ...
                    new StreamWriter("MiceliLog.txt"))
                    {
                        stringWriter.Write("\n");
                        stringWriter.Write(@"c:\Miceli Log");
                        stringWriter.Write("\n");
                        stringWriter.Write(enc.GetBytes(encrmsg));
                        stringWriter.Write("\n");
                    
                    stringWriter.Write(msgStrmPeriodOk.ToString());
                    }
					//---- Send message --------------------------------------------//
					long totalSent   = 0;
					...
					// Notify End of Data
					Core.SendLine(socket,".");

                    //250 is the OK code, so if it is not the OK code then Quit // Miceli
					reply = Core.ReadLine(socket);
					if(!IsReplyCode("250",reply)){
						OnError(SMTP_ErrorType.UnKnown,to,reply);
						Core.SendLine(socket,"QUIT");
				...
				if(supports_BDAT){
					Core.SendLine(socket,"BDAT " + (message.Length - message.Position) + " LAST");
