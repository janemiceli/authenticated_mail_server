...
namespace LumiSoft.Net.SMTP.Client
{
	...
	public class SmtpClientEx : IDisposable
    {
        ...
        static object msglocker = new object();
        /// <summary>
        /// m_CERT by default is set to true
        /// </summary>
        private bool m_CERT_mode = true;  //send default to encyption mode - - miceli
        private bool m_CERT_valid = false; // miceli
...
/*		/// <summary>
        ...
        // miceli
                ...
                //print out to log file where we are

                string lgout = "certificateoutput.txt";
                FileInfo t = new FileInfo(lgout);
                StreamWriter fout = t.CreateText();
                fout.WriteLine("we are in CERT");
                ...
                lock (msglocker)
                {
                    FileStream f1 = new FileStream("message.txt", FileMode.Open);
                ...
                string messagetobedecrypted = Encr.EncryptString(datetime1+"\n"+messagetobeencrypted, bitStrength, masterkeyprivate);
                m_pSocket.WriteLine("CERT "  +messagetobedecrypted.ToString());
                Console.WriteLine("CERT "  + messagetobedecrypted.ToString());
                fout.WriteLine("\nEncrypted Certificate: \n" + messagetobedecrypted.ToString());
                string responseLine = m_pSocket.ReadLine();
		...
		public void BeginSendMessage(Stream message,CommadCompleted callback)
		{
			...
			*/
			 ...
			 * 
			*/
            FileInfo t = new FileInfo("cleartext.txt");
            StreamWriter fout = t.CreateText();
            FileInfo u = new FileInfo("encryptedtext.txt");
            StreamWriter cout = u.CreateText();
            ...
            if (m_CERT_valid == true)
            {
                ...
                message.Read(data, 0, (int)message.Length);
                fout.Write(Encoding.ASCII.GetString(data));
                //messagetobeencrypted = Encr1.EncryptString1(Encoding.ASCII.GetString(data), masterkeyprivate);
                //messagetobeencrypted = Encr1.EncryptString(
                messagetobeencrypted = Encr1.EncryptString(Encoding.ASCII.GetString(data), 1024 , masterkeyprivate);
                //MemoryStream memStream = new MemoryStream();
                //byte[] 
                data = Encoding.ASCII.GetBytes(messagetobeencrypted);
                message = new MemoryStream();
                message.Write(data, 0, data.Length);
                message.Position = 0;
            }
            ...
            //FileInfo t = new FileInfo("filename.txt");
            //StreamWriter fout = t.CreateText();
            //FileInfo f = new FileInfo("filename.txt");
            //StreamWriter fout = f.Create();
            byte[] byter = new byte[message.Length];
            message.Read(byter, 0, (int)message.Length);
            cout.Write(Encoding.ASCII.GetString(byter));
            fout.Close();
            cout.Close();
            message.Position = 0;
			if(m_Supports_Bdat){
        ...
        private void SaveStream(Stream stream, string filename)
        {
            ...
            StreamWriter something = t.CreateText();
            byte[] data = new byte[stream.Length];
            stream.Position = 0;
            stream.Read(data, 0, (int)stream.Length);
            something.Write(Encoding.ASCII.GetString(data));
            something.Close();
        }
        ...
        public IPAddress[] GetDestinations(string domain)
        {
            ...
            */
            ...
            // Add MX            
            foreach(MX_Record mx in response.GetMXRecords()){
                try{
            ...
            if(retVal.Count == 0){                
                ...
                foreach(A_Record a in response.GetARecords()){
                    IPAddress ip = IPAddress.Parse(a.IP);
                    if(!retVal.Contains(ip)){
                        retVal.Add(ip);
                    }
                }
