...
namespace LumiSoft.Net.Dns.Client
{
	...
	public class Dns_Client
	{
		...
		{
            // TXT RR
            // |text length byte | specified count of bytes |

            byte[] text = new byte[rdLength - 1];
			Array.Copy(reply,offset + 1,text,0,rdLength - 1);
		...
		{	
			...
			*/
			...
			List<DnsRecordBase> answers = ParseAnswers(reply,answerCount,ref pos);
			List<DnsRecordBase> authoritiveAnswers = ParseAnswers(reply,authoritiveAnswerCount,ref pos);
			List<DnsRecordBase> additionalAnswers = ParseAnswers(reply,additionalAnswerCount,ref pos);
		...
		/// <returns></returns>
		private List<DnsRecordBase> ParseAnswers(byte[] reply,int answerCount,ref int offset)
		{
			...
			*/
			...
			for(int i=0;i<answerCount;i++){	
				...
				{
					case QTYPE.A:
						record = ParseARecord(reply,ref offset,rdLength,ttl);
						offset += rdLength;		
						break;

					case QTYPE.NS:
						record = ParseNSRecord(reply,ref offset,rdLength,ttl);
					//	pos += rdLength;		
						break;

					case QTYPE.CNAME:
						record = ParseCNAMERecord(reply,ref offset,rdLength,ttl);
					//	offset += rdLength;		
						break;

					case QTYPE.SOA:
						record = ParseSOARecord(reply,ref offset,rdLength,ttl);
					//	offset += rdLength;		
						break;

					case QTYPE.PTR:
						record = ParsePTRRecord(reply,ref offset,rdLength,ttl);							
					//	offset += rdLength;
						break;

					case QTYPE.HINFO:
						record = ParseHINFORecord(reply,ref offset,rdLength,ttl);							
					//	offset += rdLength;
						break;

					case QTYPE.MX:
						record = ParseMxRecord(reply,ref offset,ttl);
						break;

					case QTYPE.TXT:
						record = ParseTXTRecord(reply,ref offset,rdLength,ttl);
						offset += rdLength;
						break;

					case QTYPE.AAAA:
						record = ParseAAAARecord(reply,ref offset,rdLength,ttl);
						offset += rdLength;		
						break;
