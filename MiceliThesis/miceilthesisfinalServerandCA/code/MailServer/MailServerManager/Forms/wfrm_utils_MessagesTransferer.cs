    ...
    {     
        ...
        private string[] GetSourceFolders()
        {
            ...
            // IMAP
                ...
                try{
                    IMAP_NamespacesInfo namespacesInfo = imap.GetNamespacesInfo();
                    if(namespacesInfo.OtherUsersNamespaces != null){
                        foreach(IMAP_Namespace n in namespacesInfo.OtherUsersNamespaces){                                    
                            excludeFolders.Add(n.Name);
                        }
                    }
                    if(namespacesInfo.SharedNamespaces != null){
                        foreach(IMAP_Namespace n in namespacesInfo.SharedNamespaces){ 
                            excludeFolders.Add(n.Name);
                        }
                    }
                }
                ...
                foreach(string folder in imap.GetFolders()){
                    // See if this folder must be excluded.
                    bool exclude = false;
                    foreach(string excludeFolder in excludeFolders){                                
                        if(folder.ToLower().StartsWith(excludeFolder.ToLower())){                                    
                            exclude = true;
                            break;
                        }
                    }
        ...
        private string[] GetSourceMessages(string folder)
        {
            ...
            // IMAP
                ...
                seq.Parse("1:*");
                IMAP_FetchItem[] sourceMessages = imap.FetchMessages(seq,IMAP_FetchItem_Flags.UID,false,false);

                List<string> retVal = new List<string>();
                foreach(IMAP_FetchItem item in sourceMessages){
                    retVal.Add(item.UID.ToString());
                }
                return retVal.ToArray();
            }
