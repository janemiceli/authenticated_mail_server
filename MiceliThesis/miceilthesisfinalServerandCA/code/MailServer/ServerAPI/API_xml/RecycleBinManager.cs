...
namespace LumiSoft.MailServer
{
    ...
    internal class RecycleBinManager
    {
        ...
        {            
            ...
            try{
                // Parse message
                Mime m = Mime.Parse(messageFile);
                
                // Construct envelope
                envelope = IMAP_Envelope.ConstructEnvelope(m.MainEntity);
                        ...
                        if(line.StartsWith("\0")){
                            ...
                            if(row[0] == messageID){
                                ...
                                using(FileStream stream = File.OpenRead(m_RecycleBinPath + messageID + ".eml")){
                                    ...
                                    api.StoreMessage("system",user,folder,stream,DateTime.Now,IMAP_MessageFlags.Recent);
                                }
